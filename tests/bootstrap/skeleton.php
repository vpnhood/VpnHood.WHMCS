 ...
 * because WHMCS localAPI is unavailable under PHP-CLI on this box. It ensures
 * everything listed in fixtures.json exists — product group, products (+pricing
 * and store slugs), test clients (+client-area users, + reseller credit), and
 * the Partner Hub partner record with its product mappings.
 *
 * Create-if-missing only: it never deletes or overwrites existing fixture data,
 * except (a) topping the reseller's credit up to the spec minimum and (b)
 * re-hashing the partner secret when it does not match the one supplied.
 *
 * Usage: php skeleton.php /path/to/fixtures.json
...
$fixturesPath = $argv[1] ?? fail('usage: php skeleton.php <fixtures.json>');
$spec = json_decode(file_get_contents($fixturesPath), true) ?: fail("cannot parse $fixturesPath");
...
foreach ($spec['products'] as $p) {
    ...
    if ($prod) {
        ...
        $pid = insertRow($db, 'tblproducts', [
            ...
            'servertype' => $p['servertype'], 'welcomeemail' => $p['welcomeemail'],
    ...
    if (cols($db, 'tblproducts_slugs') &&
        ...
        insertRow($db, 'tblproducts_slugs', [
            'product_id' => $pid, 'group_id' => $gid, 'group_slug' => $g['slug'],
            'slug' => $p['slug'], 'active' => 1,
            'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s'),
        ]);
        $report['created'][] = "store slug '{$g['slug']}/{$p['slug']}'";
    }
}
...
foreach ($spec['clients'] as $role => $c) {
    $row = one($db, 'SELECT id, credit FROM tblclients WHERE email=?', [$c['email']]);
    if ($row) {
        $cid = (int)$row['id'];
